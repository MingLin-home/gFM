...
# Create a new gFM solver. The solver is of rank rank_k. We choose the regularizer lambda_M and lambda_w to be the twice of the norm of ground-truth. The learning rate by default is 1.
# Only rank_k must be specified here.
my_gFM_solver = gFM.BatchSolver(rank_k=k,
                                lambda_M=numpy.linalg.norm(U_true) ** 2 * 2,
                                lambda_w=numpy.linalg.norm(w_true) * 2, learning_rate = 1)

# Initialization stage of the gFM. Iterate 20 loops in the initialization stage.
my_gFM_solver.initialization(X.T,y.flatten(),max_init_iter=20)
...
for t in xrange(T):
    ...
    my_gFM_solver.iterate_train(X.T, y.flatten(),max_iter=1)
